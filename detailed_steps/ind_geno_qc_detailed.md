...
This repository contains scripts, utilities, for running a sample variant quality control (QC) pipeline, including ancestry prediction and per-chromosome QC reporting. It is used to generate a Docker image, which can then be ported across systems. The Docker image was then converted to a Singularity container (.sif file) using Apptainer.
...
- Edit `parameters.txt` to change input/output paths, QC thresholds, and pipeline options.
- See `scripts/` and `utils/` for individual step scripts and R Markdown reports.
...
- For Docker issues, verify Docker is running and you have permission to use it.
...
### SAMPLE_VARIANT_QC_SAMPLE_VARIANT_QC_RUNNER.sh
...
For reference, the script for converting hg38 to hg37 is also present in the utils directory but it is NOT USED BY THIS PIPELINE.
    ...
    5. If the overlap between the study and hg38 reference .bim file is ≥80%, it is in hg38. No further steps are performed.
    6. If the overlap between the study and hg37 reference .bim file is ≥80%, the study data is in hg37. This information is passed back to the calling bash script, and liftover is subsequently performed.
    7. **Enhanced Build Detection**: If the overlap is less than 80% with both hg37 and hg38, the pipeline employs progressive MAF filtering strategies:
       - **MAF > 1% Strategy**: Creates a temporary dataset containing variants with MAF > 1% only and re-tests build detection
       - **MAF > 5% Strategy**: If 1% fails, creates a dataset containing variants with MAF > 5% only and re-tests
       - **Failure Handling**: If all strategies fail, provides detailed diagnostics and recommendations
    8. **rsID Preservation**: The pipeline preserves existing rsIDs in column 2 of .bim files when available, maintaining compatibility with downstream analyses while updating coordinates if needed. 
...
For samples, we get the sample call rate and heterozygosity rate, and plot the histograms of the distribution. The reported vs genetic sex is compared (with graceful handling when sex-check data is unavailable).
...
#### Step 5: SNP Intersection and LD Pruning
    ...
    _Reference: Step5_SNPIntersectForPCA.sh_
